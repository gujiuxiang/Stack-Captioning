...
def train(opt):
    ...
    while True:
        ...
        if opt.caption_model == 'stack_cap':
            ...
            if not sc_flag:
                print("{}|I:{}/E:{}|Train_loss:{:.3f}|T/B={:.3f}".format(opt.caption_model, iteration, epoch, loss.data[0], end - start))
            else:
                print("{}|I:{}/E:{}|Avg_reward:{:.3f}|T/B={:.3f}".format(opt.caption_model, iteration, epoch, np.mean(reward[:, 0]), end - start))
        ...
        if (iteration % opt.losses_log_every == 0):
            if tf is not None:
                if opt.caption_model == 'stack_cap':
                    add_summary_value(tf_summary_writer, 'train_loss_coarse', loss_coarse.data[0], iteration)
                    add_summary_value(tf_summary_writer, 'train_loss_fine_0', loss_fine_0.data[0], iteration)
                    add_summary_value(tf_summary_writer, 'train_loss_fine_1', loss_fine_1.data[0], iteration)
                else:
        ...
        if (iteration % opt.save_checkpoint_every == 0):
            ...
            val_loss, predictions, lang_stats = eval_utils.eval_split(opt, model, crit, loader, eval_kwargs)
